    ...
    switch (mode)
        {
            // Translation
            case MV_TRANS:
                i       = rand() % tot_beads_glb;  // Pick random bead
                i       = bead_info_glb[i][BEAD_CHAINID]; // Get the chainID
...
int Move_Rot(int beadID, float MyTemp)
{
    ...
    CandNum = Check_RotStatesOld(beadID, resi, MyTemp);
    OP_NormalizeRotState(0, CandNum);

    yTemp = OP_PickRotState(CandNum);
    ...
    // There is no need to calculate the Rosenbluth weights for this case, because FW and BW are the same.
    lLDub MHAcc = OP_GenMHValue(0., 0., 0., (lLDub) MyTemp);
    ...
    for (i = firstB; i < lastB; i++)
        {
            ...
            BSum += MC_RosenbluthSampling_ForChains_AtOld(i, resi, &oldEn, old_ovlp_num); // Calculate the Rosenbluth weights. Summation in log space means multiplication in linear space.
        }
...
{ // Performs a double-pivot move.
  ...
  linker_len_glb[i']. We'll count how many candidates there are and select one
    ...
    for (i = 0; i < beadNum; i++)
        {
            ...
            Energy_Iso_ForRange(tmpBead, smBead, lgBead, &newEn, &oldEn, &new_ovlp_num, &new_cont_num,
                                naNewOvlpNeighs_glb, naNewContNeighs_glb);
            FSum += MC_RosenbluthSampling_ForRange_AtNew(tmpBead, resi, smBead, lgBead, &yTemp, &newEn, new_ovlp_num);
            if (yTemp != -1)
                {
                    resj                              = bead_info_glb[yTemp][BEAD_TYPE];
                    bead_info_glb[yTemp][BEAD_FACE]   = tmpBead;
...
int Check_RotStates_wNeighList_ForChains(int const beadID, int const resi, const int* neighList, int const neighNum)
{
    int CandNums = 0;
    int tmpBead, tmpfaceID, firstbeadID, lastbeadID, chainID;
    int resj, i;

    chainID     = bead_info_glb[beadID][BEAD_CHAINID];
    firstbeadID = chain_info_glb[chainID][CHAIN_START]; // First bead of the chain where beadID is.
    lastbeadID  = firstbeadID + chain_info_glb[chainID][CHAIN_LENGTH];

    for (i = 0; i < neighNum; i++)
        {
            tmpBead = neighList[i];
            resj    = bead_info_glb[tmpBead][BEAD_TYPE];
            // Check if the tmpBead is a preceding part of the chain.
            // In this case, the tmpBead already decided that it did not bond to any bead.
            // So, it can not be a candidate.
            if (tmpBead >= firstbeadID && tmpBead < beadID) 
                {
                    continue;
                }
            if (faEnergy_glb[resi][resj][E_SC_SC] == 0.f)
            ...
            tmpfaceID = bead_info_glb[tmpBead][BEAD_FACE];
            // Check if the tmpfaceBead is a remained part of the chain.
            // if the tmpfaceBead is a later checked bead, then tmpBead can be a candidate
            // because the bond between tmpBead and tmpfaceBead was not formed 
            // before deciding the beadID's bond.
            if (tmpfaceID == -1 || ( (tmpfaceID >= beadID) && (tmpfaceID < lastbeadID) ) )
                {
                    ldaBoltzFac_glb[CandNums]   = ldaBoltzFacNorm_glb[resi][resj];
                    naRotTrial_glb[0][CandNums] = tmpBead;
                    CandNums++;
...
int Check_RotStates_wNeighList_ForLists(int const beadIdx, const int listSize, const int beadList[MAX_BONDS + 1], int const resi, const int* neighList, int const neighNum)
{
    int CandNums = 0;
    int tmpBead, tmpfaceID;
    int resj, i, j;
    int beadID = beadList[beadIdx];

    for (i = 0; i < neighNum; i++)
        {
            tmpBead = neighList[i];
            resj    = bead_info_glb[tmpBead][BEAD_TYPE];
            // Check if the bead is already in the list.
            // If it is, then it cannot be a candidate because it
            // already decided that it did not bond to any bead.
            for (j = 0; j < beadIdx; j++) 
                {
                    if (tmpBead == beadList[j])
                        {
                            break;
                        }
                }
            ...
            if (faEnergy_glb[resi][resj][E_SC_SC] == 0.f)
                {
                    continue; // No interaction between these two types.
                }
           
            tmpfaceID = bead_info_glb[tmpBead][BEAD_FACE];

            // check if the tmpfaceBead is a remained part of the list.
            for(j= beadIdx; j < listSize; j++) 
                {
                    if (tmpfaceID == beadList[j])
                        {
                            break;
                        }
                }
            // j < listSize means that the tmpfaceID is a remained part of the list.
            // if the tmpfaceID is a later checked bead, then tmpBead can be a candidate
            if (tmpfaceID == -1 || j < listSize )  
                {
                    ldaBoltzFac_glb[CandNums]   = ldaBoltzFacNorm_glb[resi][resj];
                    naRotTrial_glb[0][CandNums] = tmpBead;
                    CandNums++;
...
/// @brief this function is used to scan the roational states of the beadID 
///        in the sequencial order of the given range.
/// @param beadID 
/// @param resi 
/// @param smallestBead means the bead which has the smallest beadID in the Range
/// @param largestBead means the bead which has the largest beadID in the Range
/// @param neighList 
/// @param neighNum 
/// @return 
int Check_RotState_wNeighList_ForRange(int const beadID, int const resi, int const smallestBead, int const largestBead, const int* neighList, int const neighNum)
{
    int CandNums = 0;
    int tmpBead, tmpfaceID;
    int resj, i;
    ...
    for (i = 0; i < neighNum; i++)
        {
            tmpBead = neighList[i];
            resj    = bead_info_glb[tmpBead][BEAD_TYPE];
            // Check if the tmpBead is a preceding part of the range.
            // In this case, the tmpBead already decided that it did not bond to any bead.
            // So, it can not be a candidate.
            if (tmpBead >= smallestBead && tmpBead < beadID) 
                {
                    continue;
                }
            if (faEnergy_glb[resi][resj][E_SC_SC] == 0.f)
            ...
            tmpfaceID = bead_info_glb[tmpBead][BEAD_FACE];
            // Check if the tmpfaceBead is a remained part of the range.
            // if the tmpfaceBead is a later checked bead, then tmpBead can be a candidate
            // because the bond between tmpBead and tmpfaceBead was not formed
            // before deciding the beadID's bond.
            if (tmpfaceID == -1 || ( (tmpfaceID >= beadID) && (tmpfaceID <= largestBead) ) )
                {
                    ldaBoltzFac_glb[CandNums]   = ldaBoltzFacNorm_glb[resi][resj];
                    naRotTrial_glb[0][CandNums] = tmpBead;
                    CandNums++;
...
/// A dissociated state integrated into the cumulative distribution implicitly.
/// The normalized Boltzmann factor for the dissociated state is considered to be
/// appended to the end of the ldaBoltzFac_glb[] array. This is because the
/// dissociated state is always a possible state.
void OP_NormalizeRotState(const int beadVal, const int CandNum)
{
    int i;

    ldaBoltzNorm_glb[beadVal] = 1.; // Default value is a dissociated state

    for (i = 0; i < CandNum; i++)
        {
            ldaBoltzNorm_glb[beadVal] += ldaBoltzFac_glb[i]; // Rosenbluth weights for beadVal
        }
    for (i = 0; i < CandNum; i++)
        {
...
/// \param CandNum - the number of possible bonding candidates.
/// \return The new proposed bonding partner.
int OP_PickRotState(int CandNum)
{
    int newRot = -1;
    int i;
    ...
    // if i == CandNum, it means that all the states are unchosen and the
    // last state, which is the dissociated state, is selected.
    if (i < CandNum) 
    {
...
lLDub MC_RosenbluthSampling_ForLocal_AtOld(const int beadID, const int resi, long double* oldEn, const int neigh_num)
{
    ...
    if (nBeadTypeIsSticker_glb[resi])
        {
            // the anisotropic energy should not be added to the oldEn
            // because it is already counted in this biased smapling.
    ...
    if (nBeadTypeIsSticker_glb[resi])
        {
            ...
            ros_num = Check_RotStates_wNeighList_ForChains(beadID, resi, naOldOvlpNeighs_glb, neigh_num);
            OP_NormalizeRotState(0, ros_num);
            return logl(ldaBoltzNorm_glb[0]);
        }
    else
                                            ...
                                            const int neigh_num)
{
    ...
    *bead_part         = -1;
    if (nBeadTypeIsSticker_glb[resi])
        {
            // OP_ShuffleArray(neigh_num, naNewOvlpNeighs_glb);
            // ros_num = Check_RotStates_wNeighList_ForChains(beadID, resi, naNewOvlpNeighs_glb, neigh_num);
            ros_num = Check_RotStates_wNeighList_ForChains(beadID, resi, naNewOvlpNeighs_glb, neigh_num);
            OP_NormalizeRotState(0, ros_num);
            if (cur_part == -1)
                {
    ...
    if (nBeadTypeIsSticker_glb[resi])
        {
            ...
            // ros_num = Check_RotStates_wNeighList(beadID, resi, naOldOvlpNeighs_glb, neigh_num);
            ros_num = Check_RotStates_wNeighList_ForLists(beadIdx, listSize, beadList, resi, naOldOvlpNeighs_glb, neigh_num);           
            OP_NormalizeRotState(0, ros_num);
            return logl(ldaBoltzNorm_glb[0]);
        }
    else
    ...
    if (nBeadTypeIsSticker_glb[resi])
        {
            // the anisotropic energy should not be added to the oldEn
            // because it is already counted in this biased smapling.
            // *newEn  = *newEn + Energy_Anisotropic_For_List(beadID, listSize, beadList);
            // ros_num = Check_RotStates_wNeighList(beadID, resi, naNewOvlpNeighs_glb, neigh_num);
            ros_num = Check_RotStates_wNeighList_ForLists(beadIdx, listSize, beadList, resi, naNewOvlpNeighs_glb, neigh_num);           
            OP_NormalizeRotState(0, ros_num);
            if (cur_part == -1)
                {
                    *bead_part = OP_PickRotState(ros_num);
    ...
    if (nBeadTypeIsSticker_glb[resi])
        {
            ...
            // ros_num = Check_RotStates_wNeighList(beadID, resi, naOldOvlpNeighs_glb, neigh_num);
            ros_num = Check_RotState_wNeighList_ForRange(beadID, resi, smallestBead, largestBead, naOldOvlpNeighs_glb, neigh_num);
            OP_NormalizeRotState(0, ros_num);
            return logl(ldaBoltzNorm_glb[0]);
        }
    else
...
lLDub MC_RosenbluthSampling_ForRange_AtNew(const int beadID, const int resi, const int smallestBead, 
                                           const int largestBead, int* bead_part, lLDub* newEn, const int neigh_num)
{
    int ros_num;
    const int cur_part = bead_info_glb[beadID][BEAD_FACE];
    *bead_part         = -1;
    if (nBeadTypeIsSticker_glb[resi])
        {
            // OP_ShuffleArray(neigh_num, naNewOvlpNeighs_glb);
            // ros_num = Check_RotStates_wNeighList(beadID, resi, naNewOvlpNeighs_glb, neigh_num);
            ros_num = Check_RotState_wNeighList_ForRange(beadID, resi, smallestBead, largestBead, naNewOvlpNeighs_glb, neigh_num);
            OP_NormalizeRotState(0, ros_num);
            if (cur_part == -1)
                {
                    *bead_part = OP_PickRotState(ros_num);
